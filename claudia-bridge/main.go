...
func startRESTServer(client *whatsmeow.Client, messageStore *MessageStore, port int) {
	...
	// Start the server — loopback only. Every real caller (whatsapp-mcp-server,
	// doctor.sh, finance-price-watch.py) already talks to localhost:8080; binding
	// to all interfaces served no purpose except letting anything on the LAN/tailnet
	// send WhatsApp messages as this account with zero auth. Confirmed via grep
	// across whatsapp-mcp/secondbrain-automation/secondbrain-dashboard/secondbrain-plugin
	// on 2026-08-02 — no caller uses anything but "localhost".
